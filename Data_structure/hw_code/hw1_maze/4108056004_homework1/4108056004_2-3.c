...
//#include<iostream>
//#include<string.h>
//using namespace std;                  //與前幾題有許多相同地方 針對不同的地方做注釋 
...
int main(){
	...
	while(numtest<=100){
		...
		element stack[n*n*floor],p_stack[n*n*floor];      //偷偷用一下stack算P 
		...
		int p_count=0;
		int pend1_f,pend1_r,pend1_c,pend2_f,pend2_r,pend2_c;    //p點端點
		int space=0;
			...
			for(j=space;j<n;j++){
				...
				while((ch=getc(fptr))!='\n'){       //輸入 
					if(ch!=32){                    //去掉空格 	
						...
						if(ch=='P'){                       
							p_stack[p_count].f=i;         //記錄所有P點 
							p_stack[p_count].r=j;
							p_stack[p_count].c=k;
							p_count++;
							maze2[i][j][k]='X';           //將所有P當作牆壁來看(除了端點 
							maze[i][j][k]='X';
						}
						//printf("%d %d %d: %c\n",i,j,k,ch);
						k++;
		...
		maze2[pend2_f][pend2_r][pend2_c]='O';            //將端點在設成O點 
		...
		while(top>-1){
			...
			if(now_f==pend1_f && now_r==pend1_r && now_c==pend1_c &&through_p==0){        //寫法是以確定有P兩端點的基礎上所做的優化設計 
				now_f=pend2_f;                                                         //若P點無端點，則以窮舉法測是否走過所有P點 
				now_r=pend2_r;                                  //但鑒於第一題已經有若以窮舉法將跑到無敵久的情況，所以這題不敢以全窮舉法來寫 
				now_c=pend2_c;
				find=1;
				now_d=5;
				path+=p_count-2;								//path直接就是P數量-2直接加(去掉端點不算) 
				through_p++;								//這邊是以類似傳送門的概念來寫 
			...
			while(now_d<=5&&find==0){           //dir1~5
				...
				if(now_d==0){
					...
					now_f=stack[top].f+1;								//較麻煩的是P也可當作U、P來看 
					now_r=stack[top].r-1;
					now_c=stack[top].c-1;
			...
			if(now_d==6){                                                         //倒退機制 
				if((stack[top].f==pend1_f&&stack[top].r==pend1_r&&stack[top].c==pend1_c)||(stack[top].f==pend2_f&&stack[top].r==pend2_r&&stack[top].c==pend2_c)){
					...
					maze2[stack[top-1].f][stack[top-1].r][stack[top-1].c]='O';     //先確認是否是傳送點 //因為傳送點已經先設為O了 
					top-=2;															//因為是以傳送為基礎概念，直接退到進第一個傳送點前的位置 
					path-=p_count-1; 
					through_p=0;
