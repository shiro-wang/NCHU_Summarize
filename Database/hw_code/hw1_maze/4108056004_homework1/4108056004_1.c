...
int main(){
	...
	while(find==1){                    //廣度搜尋 
		...
		for(i=0;i<n;i++){   
			for(j=0;j<n;j++){
				if(nmaze[i][j]==count){                   ///每次依序找位置填數字 
					if(i==end_r&&j==end_c){                      
						find_2=1;
						break;
					}
					if(i-1>=0&&(nmaze[i][j]<nmaze[i-1][j])&&maze[i-1][j]!='X'){  //必須比下一位小才頂替 
						nmaze[i-1][j]=count+1;
						find=1;
					}
	...
	if(find_2){
		...
		while(!findS){                       //找到前while 
			...
			int nummin=99,numi=-1;                     //因是從小到大的填，從終點往回找找4周最小的數走 
			for(i=0;i<4;i++){
			//	printf("temp%d = %d\n",i,temp[i]);      //check
				if(temp[i]<nummin){
